...
func main() {
	...
	// In order to use out pool of workers we need to send them work
	// and collect their results. We make 2 channels fo this
	jobs := make(chan int, 100)
	results := make(chan int, 100)

	// This starts up 3 workers, initially blocked because there are no
	// jobs yet.
	for w := 1; w <= 3; w++ {
